...
/**
 * A2A HTTP server for the Restaurant Finder agent.
 */
...
export function attachHandlers(
  ...
  app.post('/', async (req: Request, res: Response) => {
    ...
    if (uiEvent) {
      ...
      switch (action) {
        ...
        case 'confirm_booking': {
          const bookingData = JSON.stringify(ctx);
          query = `User submitted a booking: ${bookingData}`;
          break;
        }
        default:
    ...
    if (!query) {
      query = 'Show me some restaurants';
    }

    console.log(
      `[RestaurantAgentExecutor] Received query: "${query}", useUI: ${useUI}`,
    );
